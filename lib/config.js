...
async function initConfig() {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      type: 'list',
      name: 'defaultFormat',
      message: 'Default output format for read command:',
  ...
  const config = {
    ...
    defaultFormat: answers.defaultFormat || 'text'
  };
...
function getConfig() {
  ...
  const envDefaultFormat = process.env.CONFLUENCE_DEFAULT_FORMAT;
  ...
  if (envDomain && envToken) {
    ...
    if (envDefaultFormat) {
      const normalizedFormat = envDefaultFormat.toLowerCase();
      if (['text', 'markdown', 'html'].includes(normalizedFormat)) {
        defaultFormat = normalizedFormat;
      } else {
        console.warn(chalk.yellow(`⚠️  Invalid CONFLUENCE_DEFAULT_FORMAT "${envDefaultFormat}". Using "text" as default. Valid values: text, markdown, html`));
      }
    }
  ...
  try {
    ...
    let defaultFormat = 'text';
    if (storedConfig.defaultFormat) {
      const normalizedFormat = storedConfig.defaultFormat.toLowerCase();
      if (['text', 'markdown', 'html'].includes(normalizedFormat)) {
        defaultFormat = normalizedFormat;
      } else {
        console.warn(chalk.yellow(`⚠️  Invalid defaultFormat "${storedConfig.defaultFormat}" in config file. Using "text" as default. Valid values: text, markdown, html`));
      }
    }
